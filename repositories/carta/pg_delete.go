...
func Pg_Delete(idbusiness int, idcarta int) error {
	...
	_, error_carta := tx.Exec(context.Background(), query_carta, idbusiness, idcarta)
	if error_carta != nil {
		return error_carta
	}
	...
	_, error_element := tx.Exec(context.Background(), query_element, idbusiness, idcarta)
	...
	_, error_listschedulerange := tx.Exec(context.Background(), query_listschedulerange, idbusiness, idcarta)
	...
	_, error_schedule := tx.Exec(context.Background(), query_schedulerange, idbusiness, idcarta)
