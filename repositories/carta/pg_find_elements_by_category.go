...
func Pg_Find_Elements_ByCategory(idcarta int, idbusiness int, idcategory int) ([]models.Pg_Element_With_Stock_External, error) {
	...
	q := "SELECT idelement,idbusiness,idcategory,namecategory,urlphotcategory,name,price,description,urlphoto,typemoney,stock FROM element WHERE idcarta=$1 AND idbusiness=$2 AND idcategory=$3 ORDER BY stock ASC"
