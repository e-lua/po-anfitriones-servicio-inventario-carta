...
func Pg_Find_IniData(date string, idbusiness int) (models.Pg_Carta_External, error) {
	...
	error_shown := db.QueryRow(context.TODO(), q, date, idbusiness).Scan(&carta_ini_data.IDCarta, &carta_ini_data.Date, &carta_ini_data.AvailableForOrders, &carta_ini_data.Visible, &carta_ini_data.Elements, &carta_ini_data.ScheduleRanges)
