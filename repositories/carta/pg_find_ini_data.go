...
func Pg_Find_IniData(date string, idbusiness int) (models.Pg_Carta_External, error) {
	...
	q := "SELECT c.idcarta,c.date,c.availableorders,c.visible,COUNT(e.idelement),COUNT(s.idschedulerange) FROM carta c LEFT JOIN Element e on c.idcarta=e.idcarta LEFT JOIN schedulerange s on c.idcarta=s.idcarta WHERE c.date=$1 AND c.idbusiness=$2 GROUP BY c.idcarta, c.date,c.availableorders,c.visible"
