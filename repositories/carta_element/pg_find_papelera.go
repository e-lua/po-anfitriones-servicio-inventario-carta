...
func Pg_Find_Papelera(idbusiness int) ([]models.Pg_Element_Tofind, error) {
	...
	for rows.Next() {
		...
		rows.Scan(&oElement.Typefood, &oElement.IDCategory, &oElement.URLPhotoCategory, &oElement.NameCategory, &oElement.IDElement, &oElement.Name, &oElement.Description, &oElement.TypeMoney, &oElement.Price, &oElement.UrlPhoto, &oElement.Available, &oElement.SendToDelete, &oElement.Insumos, &oElement.Costo)
