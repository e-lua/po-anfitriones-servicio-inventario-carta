...
func Pg_Find_All(idbusiness int) ([]models.Pg_Category, error) {
	...
	q := "SELECT c.idcategory, COUNT(e.idelement) as elements ,c.name,c.urlphoto,c.available FROM Category c LEFT OUTER JOIN Element e on c.idcategory=e.idcategory WHERE c.idbusiness=$1 GROUP BY c.idcategory"
