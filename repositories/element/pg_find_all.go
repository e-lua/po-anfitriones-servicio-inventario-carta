...
func Pg_Find_All(idbusiness int, limit int, offset int) ([]models.Pg_Element, error) {
	...
	q := "SELECT c.typefood,c.idcategory,COALESCE(c.urlphoto,'https://restoner-public-space.sfo3.cdn.digitaloceanspaces.com/restoner-general/default-image/default-img.png'),c.name,e.idelement,e.name,e.description,e.typemoney,e.price,COALESCE(e.urlphoto,'https://restoner-public-space.sfo3.cdn.digitaloceanspaces.com/restoner-general/default-image/default-img.png'),e.available FROM element e JOIN category c on e.idcategory=c.idcategory WHERE c.idbusiness=$1 ORDER BY e.name ASC LIMIT $2 OFFSET $3"
