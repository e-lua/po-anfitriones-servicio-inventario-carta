...
func Pg_Update_Data(element models.Pg_Element, idbusiness int) error {
	...
	if _, err_update := db.Exec(context.Background(), q, element.Price, element.Description, element.TypeMoney, time.Now(), element.IDCategory, element.IDElement, idbusiness); err_update != nil {
		return err_update
	}
